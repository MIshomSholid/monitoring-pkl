...
@section('content')
    ...
    <h1 class="text-2xl font-bold mb-6">Edit Periode PKL</h1>
    ...
    <div class="bg-white border rounded-lg p-6 max-w-xl">
        ...
        <form method="POST" action="{{ route('admin.periode.update', $periode) }}">
            ...
            {{-- Nama Periode --}}
            <div class="mb-4">
                <label class="block text-sm font-medium mb-1">Nama Periode</label>

                <input type="text" name="nama_periode" value="{{ old('nama_periode', $periode->nama_periode) }}" class="w-full border rounded px-3 py-2
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tahun_ajaran" value="{{ old('tahun_ajaran', $periode->tahun_ajaran) }}"
                    pattern="\d{4}/\d{4}" title="Format harus seperti 2026/2027" class="w-full border rounded px-3 py-2
                      @error('tahun_ajaran') border-red-500 @enderror" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" rows="3"
                    class="w-full border rounded px-3 py-2">{{ old('keterangan', $periode->keterangan) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="is_active" class="w-full border rounded px-3 py-2">
                    <option value="1" {{ old('is_active', $periode->is_active) ? 'selected' : '' }}>
                        Aktif
                    </option>
                    <option value="0" {{ !old('is_active', $periode->is_active) ? 'selected' : '' }}>
                        Nonaktif
                    </option>
                </select>
            </div>
