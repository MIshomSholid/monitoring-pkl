            ...
            <div class="bg-white border rounded-lg p-4">
                <form method="GET" class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <input type="text" name="search" value="{{ request('search') }}"
                        ...
                        class="w-full sm:w-72 border rounded px-3 py-2 focus:ring focus:ring-indigo-200 text-sm" />
                    ...
                    <a href="{{ route('admin.admin-data.index') }}"
                        class="w-full sm:w-auto px-4 py-2 text-sm bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 text-center">
                        Reset
                    </a>
                </form>
            </div>
                    ...
                    <tbody class="divide-y">
                        @forelse ($admins as $admin)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">{{ $admin->no_telepon ?? '-' }}</td>
            ...
            <div class="lg:hidden space-y-3">
                @forelse ($admins as $admin)
                    <div class="bg-white border rounded-lg p-4 text-sm space-y-2">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-800">{{ $admin->nama_lengkap }}</h3>
                            <p class="text-gray-500 text-xs">{{ $admin->user->email ?? '-' }}</p>
                        </div>
                        ...
                        <div class="flex gap-4 pt-2">
                            <a href="{{ route('admin.admin-data.show', $admin->id) }}"
                                class="text-blue-600 text-sm">Detail</a>
